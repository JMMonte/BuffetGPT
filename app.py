...
risk_management = RiskManagement(max_position_size=0.1, min_position_size=0.01, max_portfolio_exposure=0.9, stop_loss=0.2, take_profit=0.2)
...
def run_bot(strategy, data, investment_amount, investment_cycle, start_date, end_date):
    ...
    while current_date <= end_date and st.session_state.running:
        try:
            ...
            adjusted_orders = risk_management.adjust_orders(orders, investment_amount)
            ...
            status = f"Date: {current_date.strftime('%Y-%m-%d')}, Orders: {adjusted_orders}"
            st.write(status)
    ...
    if run_state and not st.session_state.running:
        ...
        if ticker_list == ['']:
            ...
            for strategy_name in selected_strategies:
                ...
                adjusted_investment_log = risk_management.apply_stop_loss_and_take_profit(investment_log, data)
                portfolio_manager.update_portfolio(adjusted_investment_log[-1]['orders'])
